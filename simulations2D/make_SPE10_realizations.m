...
[G_true, W_true, rock_true] = getSPE10setup();

% Define variables
layers = G_true.cartDims(3); %total number of SPE10 layers
dims   = 60;                 %desired subset dimensions

% Predefine temporary porosity/permeability matrices for speed
[all_poro_1, all_poro_2, all_poro_3] = deal(zeros(dims*dims, layers));
[all_perm_1, all_perm_2, all_perm_3] = deal(zeros(dims*dims, size(rock_true.perm,2), layers));

% Compute individual porosity/permeability realizations and collect
for i=1:layers
    rock1 = getSPE10rock(1:dims, 1:60, i);
    all_poro_1(:,i) = rock1.poro;
    all_perm_1(:,:,i) = rock1.perm;

    rock2 = getSPE10rock(1:dims, 61:120, i);
    all_poro_2(:,i) = rock2.poro;
    all_perm_2(:,:,i) = rock2.perm;

    rock3 = getSPE10rock(1:dims, 121:180, i);
